...
# # test convergence
# thetas_perc_arr, thetas_avg_perc_arr, thetas_pegasos_arr = [], [], []
# T_arr = np.arange(10, 100, 1)
# fig = plt.figure()
# for t in range(len(T_arr)):

# 	plt.plot(T_arr[t], p1.perceptron(toy_features, toy_labels, T_arr[t])[0][0], ls='None', marker='o', ms=3, color='k', alpha=0.5, label='Perceptron' if t==0 else None)
# 	plt.plot(T_arr[t], p1.average_perceptron(toy_features, toy_labels, T_arr[t])[0][0], ls='None', marker='o', ms=3, color='red', alpha=0.5, label='Avg. Perceptron' if t==0 else None)
# 	plt.plot(T_arr[t], p1.pegasos(toy_features, toy_labels, T_arr[t], L)[0][0], ls='None', marker='o', ms=3, color='blue', alpha=0.5, label='Pegasos' if t==0 else None)

# plt.legend(title='Classification Algorithm', frameon=True)
